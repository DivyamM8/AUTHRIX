...
setupform && setupform.removeEventListener('submit' ,e => {
    e.preventDefault();
    ...
    .then(data =>{
        if (data.error){
            showMessage(document.getElementById('setup-error'), DataTransfer.error, true);
        } else {
            showMessage(document.getElementById('setup-error'), "Master Password Set !! Please Login.")
...
document.getElementById('setup-password').addEventListener('input', e=> {
    ...
    document.getElementById('seutp-strenght').textContent = 'Strength' + strengthText(score)
});
